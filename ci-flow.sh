...
+-+-+-+-+-+-+-+'
    ...
    echo docker run --rm -v $PWD:/workspaces/${PROJECT} -p 8080:8080 \
        -e AWS_PROFILE=${AWS_PROFILE} -e AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID} \
        -e AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY} \
        --name ${PROJECT}-dev-tools biswalaws/tlx-dev-tools-new \
...
show_tools_mvn()
{
    ...
    docker run --rm biswalaws/tlx-dev-tools-new bash -c "java --version; mvn --version; aws --version; git --version"
}

show_tools_yarn()
{
    echo docker run --rm biswalaws/tlx-dev-tools-new bash -c "git --version; node --version; yarn --version"
    docker run --rm biswalaws/tlx-dev-tools-new bash -c "git --version; node --version; yarn --version"
}
...
prepare_release()
{
    ...
    docker run --rm -v $PWD:/workspaces/${PROJECT} -v $HOME/.m2:/root/.m2 -p 8080:8080 \
        ...
        biswalaws/tlx-dev-tools-new bash -c "cd /workspaces/${PROJECT}/${PROJECT_PACKAGE_DIR}; ls -lrt; git config --global --add safe.directory /workspaces/${PROJECT}; git remote -v; npx semantic-release" 2> ${OUT_SEMANTIC_RELEASE}
